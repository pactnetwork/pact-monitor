...
| 2 | [`compute-broker/`](./compute-broker/) | `@0gfoundation/0g-compute-ts-sdk` round-trips: deposit → list provider → `getRequestHeaders()` → POST → `processResponse()` | 🟡 partial — discovery passes, inference POST blocked on funding |
