...
> Tạo ngày 2026-06-02 từ nhánh `feat/multi-network`, dựa trên đồ thị mã nguồn GitNexus (1.756 file / 16.841 ký hiệu / 251 luồng thực thi) và cấu trúc workspace hiện tại. Biểu đồ dùng Mermaid (hiển thị được trên GitHub).
...
graph TD
    ...
    subgraph Network["PACT NETWORK — đường ray tổng quát"]
        WRAP[wrap / wrap-v2<br/>bảo hiểm theo lệnh gọi fetch]
        SHARED[shared<br/>cổng ChainAdapter + types]
        SET[settler / settler-v2<br/>gửi settle_batch]
        IDX[indexer / indexer-v2<br/>thu nhận + API đọc + hoàn tiền]
...
Monorepo dùng pnpm + Turborepo. 24 package trong workspace, nhóm theo tầng:
...
| `protocol-v2-client` | Client TS cho Solana v2 (11 builder lệnh, 5 decoder) |
...
| `wrap-v2` | Wrap V2 (toán phí theo từng lệnh, đuôi vi phạm + evidenceHash cho `submit_claim`) |
| `settler` | Bộ gửi `settle_batch` từ Pub/Sub (NestJS) |
| `settler-v2` | Oracle-cranker V2 (pipeline premium gộp + claim) |
| `indexer` | Indexer theo từng lệnh + API đọc + giao hoàn tiền + xử lý reorg (NestJS) |
| `indexer-v2` | Thu nhận V2 + webhook Helius theo dõi trạng thái tài khoản + API đọc |
| `db` / `db-v2` | Schema Prisma (trạng thái pool, settlement, phần chia bên nhận, thu nhập) |
...
24 package chia thành **xương sống server** (settler/indexer/proxy trên `shared` + `wrap` + clients) và nhóm **package client / độc lập** (SDK, CLI công khai, SDK legacy). Các cạnh là phụ thuộc nội bộ thật từ mỗi `package.json` — gồm `dependencies` **và** `devDependencies` (package được bundle như CLI khai báo workspace dep ở `devDependencies` vì `bun build` gộp hết vào).
...
- **Trùng lặp thật cần để ý:** classifier SLA ở 5 package, và có hai client V2 Solana (`insurance` vs `protocol-v2-client`).
...
graph TD
    ...
    subgraph L3["Logic bảo hiểm"]
        WRAP2[wrap / wrap-v2<br/>toán phí · kiểm tra số dư · phân loại]
    end
...
## 9. Trạng thái hiện tại (2026-06-02)
...
- **Ràng buộc đã biết:** `declare_id!` của chương trình devnet == id mainnet → `settle_batch` revert `InvalidSeeds` trên devnet cho tới khi redeploy; db / db-v2 dùng chung một output Prisma (đua điều kiện khi cold-build CI).
