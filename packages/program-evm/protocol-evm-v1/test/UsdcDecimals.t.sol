...
///         `ArcConfig.EXPECTED_USDC_DECIMALS` so a decimals mismatch fails
...
contract UsdcDecimalsTest is Test {
    ...
    function requireUsdcDecimals(address token) public view {
        require(
            IERC20Metadata(token).decimals() == ArcConfig.EXPECTED_USDC_DECIMALS,
    ...
    function test_MockUsdcHasSixDecimals() public {
        ...
        assertEq(usdc.decimals(), ArcConfig.EXPECTED_USDC_DECIMALS, "USDC must be 6-decimal");
        assertEq(ArcConfig.EXPECTED_USDC_DECIMALS, 6);
    ...
    function test_GuardRevertsOnWrongDecimals() public {
        ...
        assertTrue(bad.decimals() != ArcConfig.EXPECTED_USDC_DECIMALS);
