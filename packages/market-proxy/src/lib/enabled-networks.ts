 ...
 * packages/settler/src/config/enabled-networks.ts. The market-proxy cannot
 * import from the settler package, so this mirrors it verbatim (agent-tasks#14)
 * for parity with the indexer's copy. Single source of truth within the proxy
 * for parsing PACT_ENABLED_NETWORKS, so the env schema and context bootstrap
 * gate their Solana deps the same way buildAdapterMap does. Default mirrors the
