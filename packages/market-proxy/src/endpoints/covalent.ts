...
const COVALENT_EXTRA_ALLOWED: ReadonlySet<string> = new Set(["authorization"]);

export const covalentHandler: EndpointHandler = {
  async buildRequest(req: Request, upstreamBase: string): Promise<Request> {
    ...
    const headers = buildUpstreamHeaders(req.headers, COVALENT_EXTRA_ALLOWED);
