  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-[#8a7a70]">Parametric API insurance for Solana agents — devnet</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader><CardTitle>Pool Balance</CardTitle></CardHeader>
          <CardValue>{formatUsdcShort(stats.poolBalance)}</CardValue>
        </Card>
        <Card>
          <CardHeader><CardTitle>Total Premiums</CardTitle></CardHeader>
          <CardValue>{formatUsdcShort(stats.totalPremiums)}</CardValue>
        </Card>
        <Card>
          <CardHeader><CardTitle>Total Refunds</CardTitle></CardHeader>
          <CardValue className="text-[#C9553D]">{formatUsdcShort(stats.totalRefunds)}</CardValue>
        </Card>
        <Card>
          <CardHeader><CardTitle>Calls Insured</CardTitle></CardHeader>
          <CardValue className="text-[#f5f0eb]">{stats.callsInsured.toLocaleString()}</CardValue>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card>
          ...
          <CardValue className="text-[#5A6B7A]">{stats.activeEndpoints}</CardValue>
        </Card>
        <Card>
          ...
          <TableBody>
            {calls.map((call) => (
              <TableRow key={call.id}>
                <TableCell>
                  <a href={`/agents/${call.agentPubkey}`} className="text-[#B87333] hover:underline">
                    {formatPubkey(call.agentPubkey)}
                  </a>
                </TableCell>
                <TableCell className="text-[#8a7a70]">{call.endpointName}</TableCell>
                <TableCell>
